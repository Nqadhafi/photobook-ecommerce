<template>
      ...
      <b-form @submit.prevent="submitForm">
        <b-form-group label="Code:" label-for="coupon-code">
          ...
          <b-form-text variant="secondary">
            <small>Coupon code cannot be changed after creation.</small>
          </b-form-text>
  ...
  <div v-else-if="loading">
    <b-spinner variant="primary" label="Loading..."></b-spinner> Loading coupon details...
  </div>

  <div v-else>
    <b-alert variant="danger" show>
      <h4>Error loading coupon</h4>
      <p v-if="errorMessage">{{ errorMessage }}</p>
      <p v-else>Coupon not found or an error occurred.</p>
      <b-button variant="secondary" :to="{ name: 'AdminCoupons' }">Back to List</b-button>
    </b-alert>
  </div>
</template>
  ...
  methods: {
    async fetchCoupon() {
      ...
      try {
        ...
        const couponData = await couponService.getAdminCoupon(couponId);
        this.coupon = couponData;

        // Isi form dengan data coupon
        this.form.code = couponData.code;
        this.form.discount_percent = couponData.discount_percent !== null ? parseFloat(couponData.discount_percent) : null;
        // this.form.discount_amount = couponData.discount_amount !== null ? parseFloat(couponData.discount_amount) : null; // Jika menggunakan discount_amount
        this.form.max_uses = couponData.max_uses !== null ? couponData.max_uses : null;
        this.form.max_uses_per_user = couponData.max_uses_per_user !== null ? couponData.max_uses_per_user : null;
        this.form.starts_at = couponData.starts_at || '';
        this.form.expires_at = couponData.expires_at || '';
        this.form.description = couponData.description || '';
        this.form.is_active = couponData.is_active;

      } catch (error) {
        console.error('Failed to fetch coupon details:', error);
        this.errorMessage = error.error || 'Failed to load coupon details. Please try again.';
      } finally {
        this.loading = false;
      }
    },
    ...
    async submitForm() {
      ...
      try {
        ...
        const response = await couponService.updateAdminCoupon(this.id, couponData);

        this.$bvToast.toast(`Coupon '${response.code}' updated successfully.`, {
          title: 'Success',
          variant: 'success',
          solid: true
        });
        ...
        const errorMsg = error.error || 'Failed to update coupon. Please try again.';
